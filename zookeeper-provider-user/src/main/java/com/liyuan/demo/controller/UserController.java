...
public class UserController{
    ...
    public BaseResVo query(@ApiParam(value = "id", required = true)@RequestParam Integer id, @RequestHeader(value = "Authorization") String authorization) {
        log.info("authorization:{}",authorization);
        UserVo vo = UserVo.builder().id(id).name("匿名").age(30).build();
        return BaseResVo.builder().resCode(200).resMsg("查询成功").data(vo).build();
    }
