...
2. nacos
3. zookeeper
1. 在分布式领域有个CAP定理（布鲁尔定理），它指出对于一个分布式计算系统来说，不可能同时满足以下3点：
...
### 五、Eureka源码学习
  ...
  3. eureka-core核心包中LeaseManager接口定义了服务注册（register）、续约（renew）、取消（cancel）和剔除（evict）的方法
